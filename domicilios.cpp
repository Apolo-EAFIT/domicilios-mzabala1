...
public class Main {
	...
	static void dijkstra( int inicial ){
     ...
     public static void graph()throws FileNotFoundException,IOException{
      GraphAL g = new GraphAL(14499);
         
        String cadena;
        boolean flag=false;
        String[] val=new String[3];
        
         FileReader f = new FileReader("c:/test/MapaActual.txt");
         BufferedReader b = new BufferedReader(f);
         int i=0;
         int j=0;
         while((cadena = b.readLine()) != null ) {
            if(cadena.equals("Arcos")||flag){
                flag=true;
                if(!cadena.equals("Arcos")&&!cadena.equals("")){
               // System.out.println(cadena);
                val=cadena.split(" ");
                 
                 g.addArc((val[0]), (val[1]),Integer.parseInt(val[2]));
                 //System.out.println(g.getSuccessors(val[0]));
                val=new String[3];
                j++;
                }
            } 
